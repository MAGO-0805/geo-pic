...
核心思想：每个像素的路径追踪互相独立——计算像素 A 不需要等像素 B 的结果。让多个 CPU 核心同时各算各的像素，最后拼成一张图。只要保证随机数不打架，结果和单线程一模一样。外层的 y 循环被切成若干段，每个线程领一段（比如线程 0 算 0~63 行，线程 1 算 64~127 行……）。

schedule(dynamic) 的意思是不做固定分配——谁先算完谁领下一段
